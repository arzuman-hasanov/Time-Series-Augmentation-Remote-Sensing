...
class TimeWarping(TimeSeriesTransform):
    """Apply time warping transformation using cubic spline interpolation."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i, pat in enumerate(x_np):
            for dim in range(x_np.shape[2]):
                time_warp = CubicSpline(warp_steps.flatten(), warp_steps.flatten() * random_warps[i, :, dim])(orig_steps)
                scale = (x_np.shape[1] - 1) / time_warp[-1]
                ret[i, :, dim] = np.interp(orig_steps, np.clip(scale * time_warp, 0, x_np.shape[1] - 1), pat[:, dim]).T
